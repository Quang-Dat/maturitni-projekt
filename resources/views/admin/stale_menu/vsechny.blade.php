<x-app-layout>
    <x-slot name="header" class="">
        ...
        <x-nav-link class="mb-4" :href="route('stale_menu.create')" :active="request()->routeIs('stale_menu.create')">
            {{ __('Vytvořit') }}
        </x-nav-link>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="{{ $loop->even ? 'bg-gray-100' : '' }} hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                            <a href="{{ route('stale_menu.edit', $radek->id) }}"
                                class="px-4 py-2 bg-yellow-300 text-gray-700 rounded-lg hover:bg-yellow-400 transition mr-2">
                                Upravit
                            </a>
                            <form action="{{ route('stale_menu.destroy', $radek->id) }}" method="POST" class="inline">
                                @csrf
                                @method('PATCH')
                                <button type="submit"
