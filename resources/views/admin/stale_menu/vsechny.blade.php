<x-app-layout>
    <x-slot name="header" class="">
        ...
        <x-nav-link class=" my-2 md:my-0" :href="route('stale_menu.create')" :active="request()->routeIs('stale_menu.create')">
            {{ __('Vytvořit') }}
        </x-nav-link>
            ...
            <tbody>
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="py-2 px-4 border-b border-gray-200 min-w-[235px]">
                        <a href="{{ route('stale_menu.edit', $radek->id) }}" class="bg-yellow-300 text-sm py-2 px-4 rounded inline-block">
                            Upravit
                        </a>
                        <form action="{{ route('stale_menu.destroy', $radek->id) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('PATCH')
